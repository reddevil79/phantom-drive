...
class SOAPServiceDescriptor(ServiceDescriptor):
    ...
    def generate(self, servicename, servicenumber, typemanager, methodlist, service_url, encoding):
        ...
        schema.setAttribute('xmlns:ns%d' %
                            servicenumber, 'urn:%s' % servicename)
        ...
        for typ in type_order:
            if isinstance(typ, list):
                ...
                if inner in type_dict:
                    element.setAttribute('type', 'ns%d:%s' %
                                         (servicenumber, inner.__name__))
                else:
                ...
                if inner in type_dict:
                    attribute.setAttribute('WSDL:arrayType', 'ns%d:%s[]' % (
                        servicenumber, inner.__name__))
                else:
                ...
                for k, v, props in typ['attributes']:
                    ...
                    if isinstance(v, list):
                        inner = v[0]
                        element.setAttribute('type', 'ns%d:ArrayOf%s' % (
                            servicenumber, inner.__name__))
                        element.setAttribute('minOccurs', '0')
                        element.setAttribute('nillable', 'true')
                    else:
                        if v in type_dict:
                            element.setAttribute(
                                'type', 'ns%d:%s' % (servicenumber, v.__name__))
                        else:
                            element.setAttribute(
        ...
        for m in methodlist:
            ...
            for arg in m.args():
                ...
                if isinstance(arg['type'], (list, tuple)):
                    part.setAttribute('type', 'ns%d:ArrayOf%s' % (
                        servicenumber, arg['type'][0].__name__))
                else:
                    if arg['type'] in type_dict:
                        part.setAttribute('type', 'ns%d:%s' % (
                            servicenumber, arg['type'].__name__))
                    else:
                        part.setAttribute('type', 'xsd:%s' %
                                          map_type(arg['type']))

...
use App\Http\Helpers\Help\Connect;
use ErrorException;
use Exception;
use App\Http\Helpers\Help\Attendance;
use App\Http\Helpers\Help\Device;
use App\Http\Helpers\Help\Face;
use App\Http\Helpers\Help\Fingerprint;
use App\Http\Helpers\Help\Os;
use App\Http\Helpers\Help\Pin;
use App\Http\Helpers\Help\Platform;
use App\Http\Helpers\Help\SerialNumber;
use App\Http\Helpers\Help\Ssr;
use App\Http\Helpers\Help\Time;
use App\Http\Helpers\Help\User;
use App\Http\Helpers\Help\Util;
use App\Http\Helpers\Help\Version;
use App\Http\Helpers\Help\WorkCode;
...
class ZKT_KALPER
{
    ...
    public function getUserTemplate($uid, $finger)
    {
        ...
        try {
            ...
            if ($bytes) {
                while ($bytes > 0) {
                    socket_recvfrom($this->_zkclient, $received_data, 1032, 0, $this->_ip, $this->_port);
                    array_push($this->user_data, $received_data);
                    $bytes -= 1024;
                }
                $this->session_id =  hexdec($u['h6'] . $u['h5']);
                socket_recvfrom($this->_zkclient, $received_data, 1024, 0, $this->_ip, $this->_port);
            }
            $template_data = array();
    ...
    private function getSizeTemplate()
    {
        ...
        if($command == Util::CMD_PREPARE_DATA)
        {
            $u = unpack('H2h1/H2h2/H2h3/H2h4', substr( $this->_data_recv, 0, + 8, 4));
    ...
    public function enrollUser($userid) {
        return User::zkenrolluser($this, $userid);
    }
...
// Modify the getAttendance() method in the ZKTeco class
    ...
    public function cancelCapture()
    {
        // Call the static method writeLCD of the Device class, passing $this (current instance),
        // 2 (the LCD screen identifier), and "Welcome Jmrashed" (the message to display).
        return Fingerprint::cancelCapture($this);
    }

    public function unlockDoor()
    {

        return Device::unlockDoor($this);
    }

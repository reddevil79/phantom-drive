        ...
        @forelse($recents as $recent)
            <tr>
                <td>
                    ...
                    $generic = $recent->gender == 'm' ? asset('assets/img/web/male.png') : asset('assets/img/web/female.png');
                    $profileImage = ($images->isEmpty() ? $generic : url($images[0]->getUrl('thumb')));
                    ?>
                    <a href="{{ action('MembersController@show',['id' => $recent->id]) }}"><img src="{{ $profileImage }}" width="50px"/></a>
                </td>

...
@section('content')
    ...
    <div class="rightside bg-grey-100">
        ...
        <div class="container-fluid">
                                ...
                                <div class="col-sm-8">
                                    <div class="row">
                                        <div class="col-sm-4">
                                            ...
                                            $generic = $member->gender == 'm' ? asset('assets/img/web/male.png') : asset('assets/img/web/female.png');
                                            $profileImage = ($images->isEmpty() ? $generic : url($images[0]->getUrl()));
                                            ?>
            ...
            <!--######################### Access control for the member ################################# -->
            <div class="row">
                <div class="col-md-12">
                    <div class="panel no-border ">
                        <div class="panel-title">
                            <div class="panel-head font-size-20">Access control for the member.</div>
                            ...
                            <div class="pull-right no-margin">
                                @if ($grantAccess == 'false')
                                    ...
                                    @if ($FP == 'both')
                                        @permission(['manage-gymie','manage-members','delete-member'])
                                        <button class="btn btn-danger" data-toggle="modal" data-target="#deleteModall-{{$member->id}}" data-id="{{$member->id}}">
                                            <span>Register User</span>
                                        </button>
                                        @endpermission
                                    @else
                                        @permission(['manage-gymie','manage-members','edit-member'])
                                        <a class="btn btn-success" href="{{ action('MembersController@registerFP',['id' => $member->id]) }}">
                                            {{--                                                    <span>{{$FP == 'true' ? "Success" : "Register FingerPrint"}} </span>--}}
                                ...
                                @else
                                    @permission(['manage-gymie','manage-members','edit-member'])
                                    <a class="" href="{{ action('MembersController@grantAccess',['id' => $member->id]) }}">
{{--                                        <span>Grant Door Access</span>--}}
                                        <button class="btn btn-success" data-toggle="modal" data-target="#loadingModal">
                                            <span>Grant Door Access</span>
                                        </button>
                                    </a>
                                ...
                                <!-- Modal -->
                                <div id="deleteModall-{{$member->id}}" class="modal fade" role="dialog">
                                    <div class="modal-dialog">
                                        ...
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-body">
                                                <p>Where do you want to send this user?</p>
                                            </div>
                                            <div class="modal-footer">
                                                <a class="btn btn-success" href="{{ action('MembersController@setUser',['id' => $member->id]) }}">GO</a>
                        ...
                        <div class="panel-body">
                            <table id="_payment" class="table table-bordered table-striped">
                                ...
                                <tbody>
{{--                                @foreach ($obj->area as $areas)--}}
                                    @if ($obj == null)
                                    @else
                                    <tr>
                                        <td>{{ $obj->userid ?? null }}</td>
                                        <td>{{ $obj->name ?? null  }}</td>
                                        <td>{{ $obj->cardno ?? null  }}</td>
                                        <td>{{ $obj->uid ?? null  }}</td>
                                        <td>
                                            <span class="{{ Utilities::getSubscriptionLabel ("Active") }}">Active</span>
                                        </td>
                                        <td>
...
{{--                                            @endpermission--}}
                                            ...
                                            @permission(['manage-gymie','manage-members','edit-member'])
                                            <a class="btn btn-danger" href="{{ action('MembersController@revokeUser',['id' => $obj->uid ?? null]) }}">
